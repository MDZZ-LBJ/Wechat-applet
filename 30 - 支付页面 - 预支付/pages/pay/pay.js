...
Page({
 ...
 async  __handleOrderPay(){
    try{
      ...
      //  4. 发起预支付
      const res1 = await request({ url: "/my/orders/req_unifiedorder", data: { order_number }, method: 'post', header: header })
      let pay = res1.pay
      // 5. 发起微信支付
      const res2 = await requestPayment(pay)
      //  6. 查询订单
      const res3 = await request({ url: "/my/orders/chkOrder", data: { order_number }, method: 'post', header: header })
      // 7 支付成功 跳转订单页面
